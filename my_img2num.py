...
###TRAINING
    ...
    for epoch in range(self.num_epochs):
      ...
      for batch_idx, (data, target) in enumerate(train_loader):
        ...
        loss=(self.model.backward(torch.t(target_64x10), eta=self.learning_rate).mean())
      ...
      print('train accuracy is {:.1f}% and loss is {:f}'.format(correct_epoch[-1].to(dtype=torch.float)/600, loss_train[-1]))
      ...
      print('For the epoch {:d}, test accuracy is {:.1f}% and loss is {:f}' .format(epoch, correct_epoch_test[-1].to(dtype=torch.float) / 100, loss_test[-1]))
...
### TESTING
    for batch_idx_test, (data_test, target_test) in enumerate(test_loader):
      ...
      loss_test.append(self.model.backward(torch.t(target_64x10)).mean())
    correct_epoch_test.append(correct_batch_test)
    print('test accuracy is {:.1f}% and loss is {:f}' .format(correct_epoch_test[-1].to(dtype=torch.float)/100, loss_test[-1]))
    print('end')
