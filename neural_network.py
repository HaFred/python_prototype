...
class NeuralNetwork:
  ...
  # *argv will contain first the number of inputs, then any number of hidden layers, and then number of outputs
    ...
    for arg in argv[0:-1]:
      ...
      i+=1
    for arg in argv[0:]:
      self.activations[j]=torch.ones(arg+1)
 ...
 #   elif len(inp.size())>=1: #TODO: implement a and z for this too later, I'd say write this whole function again anyway
 #     self.activations[0]=inp
 #     output_batch=torch.Tensor(self.num_outputs,inp.size()[-1])
 #     #print('batch case')
 #     for b in range(inp.size()[-1]):
 #       in_flat=torch.ones(inp[:,b].numel()+1,1)
 #       in_flat[1:,0]=inp[:,b]
 #       output=torch.Tensor()
 #       #print('input to nn.forward are:',in_flat)
 #       def sigmoid(arr): 
 ...
 #         if key==0: 
 #           #print('key0 position',in_flat,value)
 #           output=sigmoid(torch.mm(torch.t(value),in_flat)) #for input
 #           self.activations[key+1][b,:,:]=output
 #           #   if key==[final case]=output[key]=sigmoid(torch.cmul(output[key-1],value)) #dont apply sigmoid to output? you have to apply for now
 #         else: 
 #           #print('loop1')
 #           output_mod=torch.ones(output.numel()+1,1)
 #           output_mod[1:,0]=output
 #           output=sigmoid(torch.mm(torch.t(value),output_mod))
 #       output_batch[:,b]=output
 #       self.activations[key+1][b,:,:]=output
   # return output[-1]:reshape(input:size()) inc ase you want to return a 2D vector, but if it is a fully connected network, when will the output ever be 2d?
   
    return output_batch
  

  def backward(self,target,loss_criterion=None, eta):
 ...
 #  print('delta for last layer is or dE/dnet01', delta) #this corresponds to dE/dnet01
    #print('de by theta is',self.dE_dTheta)
    #problem is that dictionaries are not ordered
    #print('problem shape',self.dE_dTheta[numlayers-2])
    new_array=torch.Tensor(self.dE_dTheta[numlayers-2].size())
  # print('prob vec is ',self.activations[numlayers-2].size())
    new_array[0,:]=delta.mean(1) #for bias
  # print('self.activations[numlayers-2],torch.t(delta) are',self.activations[numlayers-2],torch.t(delta))
  ...
  # print('dE_dTheta for last layer is', self.dE_dTheta[numlayers-2])
   ...
   # print('check sizes,',relevant_weights,delta,relevant_activations)
      if loss_criterion=='MSE': delta=torch.mm(relevant_weights,delta)*diff_sigmoid(relevant_activations)
      elif loss_criterion=='CE': delta=torch.mm(relevant_weights,delta)*diff_sigmoid(relevant_activations)
    ...
    # print('problem vec is relevant_activations_prev,torch.t(delta)) ',relevant_activations_prev,torch.t(delta))
      self.dE_dTheta[numlayers-3-hid][1:,:]=torch.mm(relevant_activations_prev,torch.t(delta))/delta.size(1) #for weights
    # print('dE_dTheta for this layer is', self.dE_dTheta[numlayers-3-hid])
               
   #print('final delta is',self.dE_dTheta)
    if loss_criterion=='MSE': return MSE_loss #TODO: remove later, just for testing
    elif loss_criterion=='CE': return CE_Loss
