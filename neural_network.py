...
class NeuralNetwork:
    def __init__(self, *argv):
        ...
        self.theta = defaultdict(list)  # weights
        ...
        j = 0
        for arg in argv[0:-1]:
            self.theta[i] = (torch.randn(arg + 1, argv[i + 1]))
            # self.theta[i] = torch.Tensor(arg + 1, argv[i + 1])
        ...
        if len(inp.size()) == 1:
            ...
            for key, value in self.theta.items():
                if key == 0:
                    ...
                    # if key==[final case]=output[key]=sigmoid(torch.cmul(output[key-1],value)) #dont apply sigmoid
                    # to output? you have to apply for now
                else:
                    #  print('loop1')
    ...
    def backward(self, target, loss_criterion=None, eta=0.1, gd_bool=False):
        if loss_criterion is None: loss_criterion = 'CE'
        ...
        if loss_criterion == 'MSE':
            ...
            # here cost_prime plays as (CE+softmax)', refer to
    ...
    def FA(self, target, loss_creiterion, eta, gd_bool):
        super().__init__()

        # the feedback generated by the loss needs to be passed just as in the backward function
