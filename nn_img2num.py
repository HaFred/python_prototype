...
class NnImg2Num:
    ...
    def train(self):    # including each epoch validating testing
        ...
        ### TRAINING:    f: for the epoch-th training
        def train(epoch):
        ...
        ### TESTING:   f: one time test run in the test_loader

        def test():
            # self.model.eval()
            test_loss = 0
            correct = 0
            for data, target in test_loader:  # just like the enumerate in the my_img2num, iterating all the batch
                target_onehot = torch.zeros(target.shape[0], 10)
                for i in range(target.shape[0]): target_onehot[i, int(target[i])] = 1
                # data, target_onehot = Variable(data, volatile=True), Variable(target_onehot)
                output = self.model.forward(data)  # f: the forward here could be commented to implicitly call
